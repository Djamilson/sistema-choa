...
export type IDescriptionFormProps = {
  initialCar: ICar
}

export const DescriptionForm = ({ initialCar }: IDescriptionFormProps) => {
  const {
    errors,
    register,
    handleSubmit,
    ...
    <div className="mt-6  border bg-slate-100 p-4">
      ...
      {isEditing && (
        <form
          ...
          id={`formEditDescription-${initialCar.id}`}
        >
          <TextAreaFloat.Root
            name="description"
            label="Descrição"
          ...
          <div className="flex items-center justify-center gap-x-2">
            <Button
              form={`formEditDescription-${initialCar.id}`}
              disabled={!isValid || isSubmitting}
              type="submit"
